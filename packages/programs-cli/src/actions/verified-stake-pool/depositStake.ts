...
export async function depositStake(opts: Opts) {
  ...
  try {
    const signature = await stakePoolClient.depositStake({
      depositStake: new PublicKey(opts.stake),
      managerFeeAccount: new PublicKey(stakePool.managerFeeAccount),
      poolMint: new PublicKey(stakePool.poolMint),
      poolTokensTo: new PublicKey(opts.destination),
      referrerPoolTokensAccount: new PublicKey(opts.referrer),
      reserveStake: new PublicKey(stakePool.reserveStake),
      stakePool: new PublicKey(opts.stakePool),
      stakePoolDepositAuthority: new PublicKey(stakePool.stakeDepositAuthority),
