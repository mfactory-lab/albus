...
export async function depositSingle(opts: Opts) {
  ...
  try {
    const signature = await swapClient.depositSingle({
      authority: new PublicKey(opts.authority),
      destination: destination.address,
      minimumPoolTokenAmount: new BN(opts.minP),
      poolMint: tokenSwap.poolToken,
      sourceToken: new PublicKey(opts.token),
      sourceTokenAmount: new BN(opts.amount),
