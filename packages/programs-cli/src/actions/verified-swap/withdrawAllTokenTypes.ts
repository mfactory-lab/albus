...
export async function withdrawAll(opts: Opts) {
  ...
  const destinationA = await getOrCreateAssociatedTokenAccount(swapClient.connection, keypair, tokenSwap.mintA, keypair.publicKey)
  const destinationB = await getOrCreateAssociatedTokenAccount(swapClient.connection, keypair, tokenSwap.mintB, keypair.publicKey)

  try {
    const signature = await swapClient.withdrawAll({
      authority: new PublicKey(opts.authority),
      destinationTokenA: destinationA.address,
      destinationTokenB: destinationB.address,
      feeAccount: tokenSwap.feeAccount,
      minimumTokenAAmount: new BN(opts.minA),
      minimumTokenBAmount: new BN(opts.minB),
